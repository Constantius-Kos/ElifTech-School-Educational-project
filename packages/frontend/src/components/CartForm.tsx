import cl from '../pages/CartPage.module.css';
...
function CartForm() {
  ...
  const onSubmit = async (data: IFormInput) => {
    try {
      const hasEmptyItems = cartItems.some((item) => item.quantity <= 0);
      if (hasEmptyItems) {
        alert(
          'Пожалуйста, укажите количество для всех товаров в корзине или удалите лишние.'
        );
        return;
      }
  ...
  return (
    <form className={cl.CartForm} onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      />
      {errors.name && (
        <span className={cl.ErrorText}>{errors.name.message}</span>
      )}
      ...
      />
      {errors.email && (
        <span className={cl.ErrorText}>{errors.email.message}</span>
      )}
      ...
      />
      {errors.phone && (
        <span className={cl.ErrorText}>{errors.phone.message}</span>
      )}
      ...
      />
      {errors.address && (
        <span className={cl.ErrorText}>{errors.address.message}</span>
      )}

      <button disabled={cartItems.length === 0 || !isValid} type="submit">
        Submit
      </button>
    </form>
  );
